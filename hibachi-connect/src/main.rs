...
mod api_struct;
use api_struct::{get_orderbook_data_api_body::*, market_inventory_api_response::*};
use api_struct::{orderbook_data_response::*};
...
fn get_order_book(ob_depth: u32, ob_granularity: f64)  -> Result <(), Error> {
    ...
    // println!("{}", response);
    let parsed_struct: GetOrderbookData = serde_json::from_str(&response).expect("Failed to parse JSON");
